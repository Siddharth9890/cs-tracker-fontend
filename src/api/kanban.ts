...
export async function createColumn(columnData: any) {
  ...
  mutate(
    URL,
    (currentData: any) => {
      const board = currentData.board as any;
...
export async function createTask(columnId: string, taskData: any) {
  /**
   * Work on server
   */
...
export async function moveTask(updateColumns: Record<string, any>) {
  /**
   * Work in local
   */
  mutate(
