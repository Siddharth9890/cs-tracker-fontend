...
import  PlanPremiumIcon  from 'src/assets/icons/plan-premium-icon';
// components
...
export default function AccountBillingPlan({ cardList, addressBook, plans }: Props) {
  ...
  const [selectedAddress, setSelectedAddress] = useState<any | null>(primaryAddress);

  const [selectedCard, setSelectedCard] = useState<any | null>(primaryCard);
  ...
  const handleSelectAddress = useCallback((newValue: any | null) => {
    setSelectedAddress(newValue);
  }, []);

  const handleSelectCard = useCallback((newValue: any | null) => {
