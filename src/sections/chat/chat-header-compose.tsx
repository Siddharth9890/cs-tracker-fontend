...
type Props = {
  contacts: any[];
  onAddRecipients: (selected: any[]) => void;
};

export default function ChatHeaderCompose({ contacts, onAddRecipients }: any) {
  ...
  return (
    <>
      ...
      <Autocomplete
        ...
        renderOption={(any, recipient:any, { selected }) => (
          <li {...any} key={recipient.id}>
            <Box
              key={recipient.id}
              sx={{
                mr: 1,
        ...
        renderTags={(selected, any) =>
          selected.map((recipient:any, index) => (
            <Chip
              {...any({ index })}
              key={recipient.id}
