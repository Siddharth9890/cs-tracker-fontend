...
// types
// import { any, any, any } from 'src/types/tour';
// components
...
export default function TourFiltersResult({
  ...
  const handleRemoveTourGuide = (inputValue: any) => {
    const newValue = filters.tourGuides.filter((item:any) => item.name !== inputValue.name);
    onFilters('tourGuides', newValue);
  };

  const handleRemoveDestination = (inputValue: string) => {
    const newValue = filters.destination.filter((item:any) => item !== inputValue);
      ...
      <Stack flexGrow={1} spacing={1} direction="row" flexWrap="wrap" alignItems="center">
        ...
        {!!filters.tourGuides.length && (
          <Block label="Tour guide:">
            {filters.tourGuides.map((item:any) => (
              <Chip
                key={item.id}
                size="small"
        ...
        {!!filters.destination.length && (
          <Block label="Destination:">
            {filters.destination.map((item:any) => (
              <Chip
                key={item}
                label={item}
                size="small"
