...
// types
// import { any } from 'src/types/tour';
// components
...
export default function TourSearch({ query, results, onSearch, hrefItem }: any) {
  ...
  return (
    <Autocomplete
      ...
      renderOption={(any, tour, { inputValue }) => {
        const matches = match(tour.name, inputValue);
        const parts = parse(tour.name, matches);

        return (
          <Box component="li" {...any} onClick={() => handleClick(tour.id)} key={tour.id}>
            <Avatar
              key={tour.id}
              alt={tour.name}
              src={tour.images[0]}
