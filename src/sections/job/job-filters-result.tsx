...
// types
// import { any, any } from 'src/types/job';
// components
...
export default function JobFiltersResult({
  ...
  const handleRemoveEmploymentTypes = (inputValue: string) => {
    const newValue = filters.employmentTypes.filter((item:any) => item !== inputValue);
  ...
  const handleRemoveRoles = (inputValue: string) => {
    const newValue = filters.roles.filter((item:any) => item !== inputValue);
      ...
      <Stack flexGrow={1} spacing={1} direction="row" flexWrap="wrap" alignItems="center">
        {!!filters.employmentTypes.length && (
          <Block label="Employment Types:">
            {filters.employmentTypes.map((item:any) => (
              <Chip
                key={item}
                label={item}
                size="small"
