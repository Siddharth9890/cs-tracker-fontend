...
// types
// import { any, any, any } from 'src/types/job';
//
...
const applyFilter = ({
  ...
  if (employmentTypes.length) {
    inputData = inputData.filter((job) =>
      job.employmentTypes.some((item:any) => employmentTypes.includes(item))
    );
  }
  ...
  if (locations.length) {
    inputData = inputData.filter((job) => job.locations.some((item:any) => locations.includes(item)));
  }

  if (benefits.length) {
    inputData = inputData.filter((job) => job.benefits.some((item:any) => benefits.includes(item)));
  }
