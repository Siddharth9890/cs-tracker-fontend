...
export default function useCalendar() {
  ...
  const [view, setView] = useState<any>(smUp ? 'dayGridMonth' : 'listWeek');
  ...
  const onResizeEvent = useCallback(
    (arg: EventResizeDoneArg, updateEvent: (eventData: Partial<any>) => void) => {
      const { event } = arg;
  ...
  const onDropEvent = useCallback(
    (arg: EventDropArg, updateEvent: (eventData: Partial<any>) => void) => {
      const { event } = arg;
