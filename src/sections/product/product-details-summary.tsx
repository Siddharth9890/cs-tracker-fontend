...
// types
// import { any, any } from 'src/types/product';
//
import IncrementerButton from './common/incrementor-button';
...
export default function ProductDetailsSummary({
  ...
  const existProduct = cart.map((item:any) => item.id).includes(id);

  const isMaxQuantity =
    cart.filter((item:any) => item.id === id).map((item:any) => item.quantity)[0] >= available;
