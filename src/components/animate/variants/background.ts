...
export const varBgColor = (any?: any) => {
  const colors = any?.colors || ['#19dcea', '#b22cff'];
  const duration = any?.duration || 5;
  const ease = any?.ease || 'linear';
...
export const varBgKenburns = (any?: any) => {
  const duration = any?.duration || 5;
  const ease = any?.ease || 'easeOut';
...
export const varBgPan = (any?: any) => {
  const colors = any?.colors || ['#ee7752', '#e73c7e', '#23a6d5', '#23d5ab'];
  const duration = any?.duration || 5;
  const ease = any?.ease || 'linear';
