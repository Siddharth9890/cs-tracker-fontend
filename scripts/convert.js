...
// got this from https://github.com/neetcode-gh/leetcode/blob/main/.problemSiteData.json
const problems = [
  {
    neetcode150: true,
    blind75: true,
...
const filteredProblems = problems.filter((problem) => problem.neetcode150);

const groupedByPattern = filteredProblems.reduce((acc, problem, id) => {
  const { pattern } = problem;

  const newProblem = {
    name: problem.problem,
    solvingLink: `https://leetcode.com/problems/${problem.link}`,
    solutionLink: `https://www.youtube.com/watch?v=${problem.video}`,
    id,
  };

  if (!acc.find((group) => group.title === pattern)) {
    acc.push({
      title: pattern,
      questions: [],
    });
  ...
  const group = acc.find((group) => group.title === pattern);
  group.questions.push(newProblem);
...
fs.writeFileSync(
  "../neetcode150.json",
  JSON.stringify(groupedByPattern, null, 2),
  "utf-8"
);
